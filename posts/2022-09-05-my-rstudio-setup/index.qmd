...
One day I may dig more deeply into setting up my own very custom image by starting
  a little closer to do-it-from-scratch, but for now the `rocker/tidyverse` image 
...
This service file is a user `@.service`, so the file is named 
  ...
  and called via 
  `systemctl start --user docker-compose@tidyverse-extra.service` or
  `systemctl enable --now --user docker-compose@tidyverse-extra.service` for a
  persistent setup.
...
As a final note, this will only work if your user is part of the docker group,
  ...
  to work as root, but the docker-compose.yml file would need to have literal
  paths instead of user-relative ones. 
  
## Using RStudio

Now that all of the pieces are in place, all I have to do to access RStudio at
  home is to open my Chromebook, access my NAS PC 
  (creatively named `fileserver`), at fileserver:8787, and log in to RStudio Server.
...
For now this all works wonderfully. I don't have to muck around with dependencies
  ...
  means that it should be reproducible in case I change PCs or have some sort of
  catastrophic system failure. For instance, if I am away from home but want to 
  work on a project I can spin up the docker image directly on my Chromebook
  (or any computer with Docker) and fire away. 
  In fact, you can even run this image in a browser via Docker Playground, 
